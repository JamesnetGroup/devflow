...
namespace DevFlow.Finders.Local.Api
{
    ...
    public class ShellContextMenu : NativeWindow
    {
        ...
        private void InvokeCommand(IContextMenu oContextMenu, uint nCmd, string strFolder, Point pointInvoke)
        {
            ...
            _ = oContextMenu.InvokeCommand(ref invoke);
        }
        #endregion
        ...
        private void ReleaseAll()
        {
            ...
            {
                _ = Marshal.ReleaseComObject(_oContextMenu);
                _oContextMenu = null;
            }
            if (null != _oContextMenu2)
            {
                _ = Marshal.ReleaseComObject(_oContextMenu2);
                _oContextMenu2 = null;
            }
            if (null != _oContextMenu3)
            {
                _ = Marshal.ReleaseComObject(_oContextMenu3);
                _oContextMenu3 = null;
            }
            if (null != _oDesktopFolder)
            {
                _ = Marshal.ReleaseComObject(_oDesktopFolder);
                _oDesktopFolder = null;
            }
            if (null != _oParentFolder)
            {
                _ = Marshal.ReleaseComObject(_oParentFolder);
                _oParentFolder = null;
            }
            if (null != _arrPIDLs)
            {
        ...
        private void ShowContextMenu(Point pointScreen)
        {
            ...
            try
            {
                ...
                _ = Marshal.QueryInterface(iContextMenuPtr, ref IID_IContextMenu2, out iContextMenuPtr2);
                _ = Marshal.QueryInterface(iContextMenuPtr, ref IID_IContextMenu3, out iContextMenuPtr3);
            ...
            finally
            {
                ...
                {
                    _ = Marshal.Release(iContextMenuPtr);
                }

                if (iContextMenuPtr2 != IntPtr.Zero)
                {
                    _ = Marshal.Release(iContextMenuPtr2);
                }

                if (iContextMenuPtr3 != IntPtr.Zero)
                {
                    _ = Marshal.Release(iContextMenuPtr3);
                }
    ...
    internal static class ShellHelper
    {
        ...
        {
            return ((uint)ptr & 0x80000000) == 0x80000000 ? (uint)ptr >> 16 : ((uint)ptr >> 16) & 0xffff;
        }
